...
// Same deal as the three functions above but for localplayers					//SCS ADD START
...
static int lib_lenLocalplayers(lua_State *L)
{
	lua_pushinteger(L, MAXSPLITSCREENPLAYERS);
	return 1;
}																							//SCS ADD END - Implementation for "Expose localplayers to lua" merge request
...
int LUA_PlayerLib(lua_State *L)
{
	...
	// ditto with localplayers								//SCS ADD START
	lua_newuserdata(L, 0);
		lua_createtable(L, 0, 2);
			...
			lua_setfield(L, -2, "__len");
		lua_setmetatable(L, -2);
	lua_setglobal(L, "localplayers");						//SCS ADD END - Implementation for "Expose localplayers to lua" merge request
